 ...
 * ACube SDK Context
 */
const ACubeContext = createContext(undefined);
/**
...
function useReceipts() {
    ...
    const voidReceipt = useCallback(async (voidData) => {
        if (!sdk) {
            const receiptError = new ACubeSDKError('UNKNOWN_ERROR', 'SDK not initialized');
            setError(receiptError);
            return false;
    ...
    const returnReceipt = useCallback(async (returnData) => {
        if (!sdk) {
            const receiptError = new ACubeSDKError('UNKNOWN_ERROR', 'SDK not initialized');
            setError(receiptError);
            return null;
    ...
    const getReceipt = useCallback(async (receiptUuid) => {
        if (!sdk || !isOnline) {
            return null;
        }
        try {
    ...
    const getReceiptDetails = useCallback(async (receiptUuid, format = 'json') => {
        if (!sdk || !isOnline) {
            return null;
        }
        try {
