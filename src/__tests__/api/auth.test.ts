...
const mockGetAPIClient = getAPIClient as jest.MockedFunction<
  typeof getAPIClient
>;
const mockGetAuthClient = getAuthClient as jest.MockedFunction<
  typeof getAuthClient
>;
...
describe("Authentication Module", () => {
  ...
  describe("validateLoginCredentials", () => {
    ...
    it("should reject both invalid email and password", () => {
      ...
      expect(result.isValid).toBe(false);
      // With Zod, we get more detailed validation errors, so we check for at least 2 errors
      expect(result.errors.length).toBeGreaterThanOrEqual(2);
      expect(result.errors).toEqual([
        { code: "EMAIL_INVALID", field: "", message: "Invalid email format" },
        {
          code: "PASSWORD_TOO_SHORT",
          field: "",
          message: "Password must be at least 8 characters",
  ...
  describe("loginProvider", () => {
    ...
    it("should successfully login provider with valid credentials", async () => {
      ...
      expect(mockSecureTokenStorage.storeToken).toHaveBeenCalledWith(
        mockTokenResponse
      );
      expect(mockSecureTokenStorage.storeUserInfo).toHaveBeenCalledWith(
        mockTokenResponse.access_token
      );
      expect(mockAuthLogger.authSuccess).toHaveBeenCalledWith("provider", {
        email: validCredentials.email,
      });
    });

    it("should throw AuthenticationError for invalid credentials format", async () => {
      await expect(loginProvider("invalid-email", "")).rejects.toThrow(
        AuthenticationError
      );
      await expect(loginProvider("invalid-email", "")).rejects.toMatchObject({
        code: "INVALID_CREDENTIALS_FORMAT",
      });
      expect(mockAuthLogger.authFailure).toHaveBeenCalledWith(
        "provider",
    ...
    it("should handle API error responses", async () => {
      ...
      await expect(
        loginProvider(validCredentials.email, validCredentials.password)
  ...
  describe("loginMerchant", () => {
    ...
    it("should successfully login merchant with valid credentials", async () => {
      ...
      expect(mockAuthLogger.authSuccess).toHaveBeenCalledWith("merchant", {
        email: validCredentials.email,
      });
    });

    it("should throw AuthenticationError for invalid credentials format", async () => {
      await expect(loginMerchant("invalid-email", "")).rejects.toThrow(
        AuthenticationError
      );
    ...
    it("should handle API error responses", async () => {
      ...
      await expect(
        loginMerchant(validCredentials.email, validCredentials.password)
      ).rejects.toThrow(AuthenticationError);
      await expect(
        loginMerchant(validCredentials.email, validCredentials.password)
      ).rejects.toMatchObject({
  ...
  describe("loginCashier", () => {
    ...
    it("should successfully login cashier with valid credentials", async () => {
      ...
      expect(mockAuthLogger.authSuccess).toHaveBeenCalledWith("cashier", {
        email: validCredentials.email,
      });
    });

    it("should throw AuthenticationError for invalid credentials format", async () => {
      await expect(loginCashier("invalid-email", "")).rejects.toThrow(
        AuthenticationError
      );
    ...
    it("should handle API error responses", async () => {
      ...
      await expect(
        loginCashier(validCredentials.email, validCredentials.password)
      ).rejects.toThrow(AuthenticationError);
      await expect(
        loginCashier(validCredentials.email, validCredentials.password)
      ).rejects.toMatchObject({
  ...
  describe("isAuthenticated", () => {
    ...
    it("should return false when token validation throws error", async () => {
      ...
      mockSecureTokenStorage.isTokenValid.mockRejectedValue(
  ...
  describe("getCurrentUser", () => {
    ...
    it("should handle errors gracefully", async () => {
      mockSecureTokenStorage.getUserEmail.mockRejectedValue(
  ...
  describe("refreshToken", () => {
    ...
    it("should return null when token is expired", async () => {
      ...
      expect(mockAuthLogger.warn).toHaveBeenCalledWith(
        "Token refresh not available, user needs to re-login"
      );
    });
  ...
  describe("hasRole", () => {
    ...
    it("should handle errors gracefully", async () => {
      mockSecureTokenStorage.getUserRole.mockRejectedValue(
        new Error("Storage error")
      );
  ...
  describe("Integration scenarios", () => {
    it("should handle complete login and logout flow", async () => {
      ...
      // Login
      const loginResult = await loginMerchant(
        "test@example.com",
        "Password123!"
      );
