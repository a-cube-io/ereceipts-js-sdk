...
const log = createPrefixedLogger('NODE-MTLS');
...
try {
  ...
  log.warn('Node.js specific modules not available - mTLS functionality will be limited');
}
...
export class NodeMTLSAdapter implements IMTLSAdapter {
  ...
  constructor() {
    log.debug('Node.js mTLS adapter initialized');
  }
  ...
  async configureCertificate(certificateData: CertificateData): Promise<void> {
    ...
    try {
      ...
      log.debug('Certificate configured successfully:', {
  ...
  async getCertificateInfo(): Promise<CertificateInfo | null> {
    ...
    log.debug('Certificate info requested');
  ...
  async request<T>(requestConfig: MTLSRequestConfig): Promise<MTLSResponse<T>> {
    ...
    try {
      ...
      log.debug('mTLS request successful:', {
        status: response.status,
        statusText: response.statusText,
  ...
  async testConnection(): Promise<boolean> {
    ...
    try {
      ...
      log.debug('Testing connection to:', testUrl);
  ...
  async removeCertificate(): Promise<void> {
    if (this.configuration) {
      ...
      log.debug('Remove certificate: No certificate configured');
    }
  }
