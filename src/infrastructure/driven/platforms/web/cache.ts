...
import { compressData, decompressData } from '@/infrastructure/driven/cache/compression.adapter';
import { createPrefixedLogger } from '@/shared/utils';

const log = createPrefixedLogger('CACHE-WEB');
...
export class WebCacheAdapter implements ICacheAdapter {
  ...
  private async initialize(): Promise<void> {
    ...
    log.debug('Initializing IndexedDB cache', {
    ...
    try {
      ...
      log.debug('IndexedDB cache initialized successfully');
      this.retryCount = 0; // Reset retry count on success
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      log.debug('Failed to initialize IndexedDB', { error: errorMessage });
  ...
  private async openDatabase(): Promise<IDBPDatabase> {
    return await openDB(WebCacheAdapter.DB_NAME, WebCacheAdapter.DB_VERSION, {
      upgrade: (db, oldVersion, newVersion, transaction) => {
        log.debug('Database upgrade needed', { oldVersion, newVersion });
        this.handleUpgrade(db, oldVersion, newVersion, transaction);
      },
      blocked: () => {
        log.debug('Database blocked - another tab may be open');
      },
      blocking: () => {
        log.debug('Database blocking - will close connection');
        if (this.db) {
          this.db.close();
          this.db = null;
        }
      ...
      terminated: () => {
        log.debug('Database connection terminated unexpectedly');
        this.db = null;
      },
    });
  }
  ...
  private handleUpgrade(
    ...
    log.debug('Handling database upgrade', { oldVersion, newVersion });
    ...
    if (!db.objectStoreNames.contains(WebCacheAdapter.STORE_NAME)) {
      ...
      log.debug('Created cache store and timestamp index');
    }
      ...
      indexesToRemove.forEach((indexName) => {
        try {
          if (store.indexNames.contains(indexName)) {
            ...
            log.debug(`Removed unused index: ${indexName}`);
          }
        } catch (error) {
          // Ignore errors if indexes don't exist
          log.debug(`Warning: Could not remove index ${indexName}`, error);
        }
      });
    }

    log.debug('Database upgrade completed');
  }
  ...
  private async handleVersionConflict(): Promise<void> {
    log.debug('Handling version conflict, attempting recovery...');
    ...
    try {
      ...
      log.debug('Deleting problematic database to resolve version conflict');
      ...
      log.debug(`Retrying database initialization (attempt ${this.retryCount}/${this.maxRetries})`);
      this.db = await this.openDatabase();

      log.debug('Successfully recovered from version conflict');
      this.retryCount = 0; // Reset retry count on success
    } catch (retryError) {
      const retryErrorMessage = retryError instanceof Error ? retryError.message : 'Unknown error';
      log.debug('Recovery attempt failed', { attempt: this.retryCount, error: retryErrorMessage });
  ...
  async get<T>(key: string): Promise<CachedItem<T> | null> {
    ...
    log.debug('Getting cache item', { key });
    ...
    try {
      ...
      log.debug('Error getting cache item', { key, error });
  ...
  async setItem<T>(key: string, item: CachedItem<T>): Promise<void> {
    ...
    log.debug('Setting cache item', { key, timestamp: item.timestamp, compressed: isCompressed });
    ...
    try {
      ...
      log.debug('Error setting cache item', { key, error });
  ...
  async setBatch<T>(items: Array<[string, CachedItem<T>]>): Promise<void> {
    ...
    log.debug('Batch setting items', { count: items.length });
    ...
    try {
      ...
      log.debug('Batch operation completed', { count: items.length });
    } catch (error) {
      log.debug('Error in batch operation', { count: items.length, error });
  ...
  async getSize(): Promise<CacheSize> {
    ...
    try {
      ...
      log.debug('Error getting cache size', error);
      return {
        entries: 0,
        bytes: 0,
        lastCleanup: Date.now(),
  ...
  async getKeys(pattern?: string): Promise<string[]> {
    ...
    try {
      ...
      log.debug('Error getting cache keys', error);
      return [];
    }
  }
  ...
  private async delete(key: string): Promise<boolean> {
    ...
    try {
      ...
      log.debug('Error deleting cache item', { key, error });
      return false;
    }
  }
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    try {
      ...
      log.debug('Failed to ensure initialization', error);
