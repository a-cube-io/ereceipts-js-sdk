...
const log = createPrefixedLogger('CACHE-RN');
...
export class ReactNativeCacheAdapter implements ICacheAdapter {
  ...
  private async runMigrations(): Promise<void> {
    ...
    try {
      ...
      if (!this.hasCompressedColumn) {
        log.debug('Adding compressed column to cache table');
        ...
        log.debug('Successfully added compressed column');
      } else {
        log.debug('Compressed column already exists');
      }
      ...
      log.debug('Migration failed, disabling compression features', error);
  ...
  private async checkColumnExists(columnName: string): Promise<boolean> {
    try {
      ...
      log.debug('Table columns found', { columns: columns.map((c) => c.name) });

      return columns.some((column) => column.name === columnName);
    } catch (error) {
      log.debug('Error checking column existence', error);
      return false;
    }
  }
  ...
  async get<T>(key: string): Promise<CachedItem<T> | null> {
    ...
    log.debug('Executing get query', { sql, key });
    const results = await this.executeSql(sql, [key]);
    log.debug('Get query results', { key, hasResults: !!results });
  ...
  async set<T>(key: string, data: T): Promise<void> {
    ...
    log.debug('Setting cache item', { key });
  ...
  async setItem<T>(key: string, item: CachedItem<T>): Promise<void> {
    ...
    log.debug('Setting item with metadata', {
      key,
    ...
    log.debug('Executing setItem SQL', { key, paramsCount: params.length });
...
export class MemoryCacheAdapter implements ICacheAdapter {
  ...
  async get<T>(key: string): Promise<CachedItem<T> | null> {
    ...
    log.debug('Cache hit', { key, compressed: isCompressed });
  ...
  async setItem<T>(key: string, item: CachedItem<T>): Promise<void> {
    ...
    if (this.cache.size >= (this.options.maxEntries || 1000) && !this.cache.has(key)) {
      ...
      if (oldestKey) {
        ...
        log.debug('Removed oldest item for capacity', { oldestKey, freedBytes: oldestItemSize });
      }
    }
  ...
  async setBatch<T>(items: Array<[string, CachedItem<T>]>): Promise<void> {
    ...
    if (projectedEntries > maxEntries) {
      ...
      if (itemsToRemove.length > 0) {
        log.debug('Removed items for batch capacity', {
  ...
  async invalidate(pattern: string): Promise<void> {
    ...
    if (removed > 0) {
      log.debug('Invalidation completed', {
        pattern,
  ...
  async clear(): Promise<void> {
    this.cache.clear();
    this.totalBytes = 0;
    log.debug('Cache cleared', { entries: 0, bytes: 0 });
  }
